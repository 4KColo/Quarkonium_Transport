...
import sys
sys.path.append('/Users/Colo4K/Desktop/Thesis/Code_disso_form_rates/HQ-Evo')
import HqEvo
...
LBT = HqEvo.HqEvo(
	options={'2->2':True, '2->3':False, '3->2':False, 
				'mass':M, 'Nf':3,'Kfactor':1.0},		
	table_folder="./tables", 
	refresh_table=True	)
...
class HQ_p_update:
	...
	def update(self, HQ_Ep, Temp, time_step):
		# HQ_Ep is a 4-vector in MeV, Temp is in GeV, time_step is in fm/c
		p1 = np.array(HQ_Ep)/1000.0
		t0 = 0.0
		while t0 < time_step:
			...
			t0 += small_dt
		return p1*1000.0		# return the 4-momentum in MeV
